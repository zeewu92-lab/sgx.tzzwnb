...
import { EarthHero } from './EarthHero.jsx';
import { WorldTimeTimeline } from './WorldTimeTimeline.jsx';
  ...
  return (
    <div className="mb-2">
      ...
      <div className="pb-1.5">
        ...
        {/* 地球主視覺：地球本體用圖片素材，城市光點／名稱／時間用程式即時疊加，
            點一下光點＝設為/取消「目前位置」，跟原本點一下 hero 卡片的行為一致 */}
        {clocks.length > 0 && (
          <>
            <EarthHero clocks={clocks} homeClock={homeClock} now={now} lang={lang} t={t} onTapCity={tap} />
            <WorldTimeTimeline clocks={clocks} homeClock={homeClock} now={now} lang={lang} t={t} />
          </>
        )}
      </div>
      ...
      >
        ...
        {clocks.length === 0 ? (
          <div className="text-sm px-2 py-4" style={{ color: INK_SOFT }}>{t.emptyClocks}</div>
        ) : (
          <div className="pt-1 pb-6 flex flex-col gap-3">
            {/* 我的城市：目前位置那顆時鐘，從原本固定在最上方的 hero 卡片改成跟其他城市一起
                放進這個可捲動清單，視覺上仍用 hero 樣式跟其他城市區分開來 */}
            {homeClock && (
              <div className="flex flex-col gap-1.5">
                <span className="text-xs font-bold px-1" style={{ color: INK_SOFT }}>
                  {t.myCity || (lang === 'zh' ? '我的城市' : 'My City')}
                </span>
                <ClockRow
            ...
            {restClocks.length > 0 && (
              <div className="flex flex-col gap-1.5">
                <span className="text-xs font-bold px-1" style={{ color: INK_SOFT }}>
                  {t.addedCities || (lang === 'zh' ? '已加入城市' : 'Added Cities')}
                </span>
